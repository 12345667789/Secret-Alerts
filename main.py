...
# Configure the app's logger instead of the root logger
log_formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
app.logger.setLevel(logging.INFO)

# Add the handler for the dashboard
capture_handler = CaptureLogsHandler()
capture_handler.setFormatter(log_formatter)
app.logger.addHandler(capture_handler)

# Add a handler to also send logs to the standard output (for Cloud Logging)
...
if __name__ == '__main__':
    ...
    app.run(host='0.s0.0.0', port=int(os.environ.get('PORT', 8080)), debug=True)